...
function modifyManifest(installDirectory) {
	...
	new xml2js.Parser().parseString(fs.readFileSync(manifestPath), function (err, document) {
		...
		var service = '<service android:name="co.acoustic.mobile.push.plugin.imagecarousel.imagecarouselIntentService" />';
...
if(process.env.MCE_RN_NOCONFIG) {
    console.log(chalk.yellow.bold("Acoustic Mobile Push Image Carousel Plugin installed, but will not be auto configured because MCE_RN_NOCONFIG environment flag detected."));
    return;
}

console.log(chalk.green.bold("Setting up Acoustic Mobile Push Image Carousel Plugin"));
...
console.log('react-native link react-native-acoustic-mobile-push-imagecarousel\n');
