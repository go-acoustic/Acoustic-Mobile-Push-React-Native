...
export class SendTestEventScreen extends SubscribedComponent {
  ...
  componentDidMount() {
    this.subscriptions.push(RNAcousticMobilePushEmitter.addListener(EVENT_SUCCESS, (userInfo) => {
      this.setState({ statusColor: colors.success, statusText: `Sent events: ${this.eventsToString(userInfo)}` });
    }));

    this.subscriptions.push(RNAcousticMobilePushEmitter.addListener(EVENT_FAILURE, (userInfo) => {
      this.setState({
